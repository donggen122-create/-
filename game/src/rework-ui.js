...
export class GuardianUI {
  constructor(menu,callbacks){
    ...
    this.root.addEventListener('change',e=>{
      ...
      if(e.target.id==='sg-difficulty'){const help=this.root.querySelector('#sg-difficulty-help');if(help)help.innerHTML=difficultyHelp(e.target.value);const hr=this.root.querySelector('#sg-hard-ready');if(hr)hr.innerHTML=this.hardArea(this.state().profile,this.stage,e.target.value);}
    });
  }
  ...
  chapterView(p,passes,chapter,stages){
    ...
    const weaponGear=R.GEAR[p.equippedGear?.weapon],mode=weaponGear?weaponGear.type:(p.weaponMode||'melee'),gate=R.hardGate(p,st.id),difficulty=R.difficultyOf(p)==='hard'&&!gate.open?'normal':R.difficultyOf(p),weaponName=mode==='melee'?(p.hero==='minji'?'연필':'검'):(p.hero==='minji'?'피구공':'야구 배트');
    ...
    <div class="sg-departure"><div class="sg-hero-scene ${chapter.n==='2'?`sg-chapter-art ${p.stages.CH10?.cleared?'clean':''}`:''}"><img class="sg-hero" src="./assets/sprites/heroes/${p.hero==='minji'?'minji':'hoya'}_idle_1.png" alt="${p.hero==='hoya'?'호야':'민지'}"/>${p.activePet?petImage(p.activePet):'<span class="sg-future-pet">1-3 성공 후<br/>친구 선택</span>'}<span>${esc(weaponName)} · ${mode==='ranged'?'원거리':'근거리'} ${R.heroLocked(p)?'<button data-tab="gear" class="sg-inline">장비 보기</button>':'<button data-do="hero-dialog" class="sg-inline">캐릭터 고르기</button>'}</span></div><div class="sg-brief"><span class="sg-eyebrow">선택한 모험 · ${label}</span><h2>${esc(st.name)}</h2><p>${esc(st.story)}</p><div class="sg-goal">${icon('element_wind')} ${esc(st.goal)} <small>환경 목표 · 성공하면 코인 +30</small></div><div class="sg-options"><label>난이도<select id="sg-difficulty">${entries(R.DIFFICULTIES).map(([id,d])=>`<option value="${id}" ${difficulty===id?'selected':''} ${id==='hard'&&!gate.open?'disabled':''}>${esc(d.name)} ${starText(d.stars)}${id==='hard'&&!gate.open?' 🔒':''}</option>`).join('')}</select></label><label>기본 무기<select id="sg-weapon-mode" ${weaponGear?'disabled title="무기 장비가 공격 방식을 정해요"':''}><option value="melee" ${mode==='melee'?'selected':''}>근거리 · ${p.hero==='minji'?'연필':'검'}</option><option value="ranged" ${mode==='ranged'?'selected':''}>원거리 · ${p.hero==='minji'?'피구공':'야구 배트'}</option></select></label></div><div id="sg-difficulty-help" class="sg-weapon-help" aria-live="polite">${difficultyHelp(difficulty)}</div><div id="sg-hard-ready">${this.hardArea(p,st.id,difficulty)}</div><div id="sg-weapon-help" class="sg-weapon-help" aria-live="polite">${weaponHelp(mode,p.hero)}${weaponGear?`<span class="sg-gear-mode-note">무기 장비 <b>${esc(weaponGear.name)}</b>${subjJosa(weaponGear.name)} 공격 방식을 정해요. 장비 탭에서 무기를 바꾸면 바뀌어요.</span>`:''}</div><small>성공하면 난이도만큼 별을 받아요: 쉬움 ★ · 보통 ★★ · 어려움 ★★★. 성공 보급권은 쉬움·보통 1장, <b>어려움 2장</b>. 같은 단계는 하루 2번 성공까지 받아요(아침 8시에 다시).</small>${passes?.day?`<div class="sg-goal sg-stage-gift-note">${icon('gift')} 오늘 이 단계 보급권: ${R.stageGiftLeft(p,st.id,passes.day)?`<b>${R.stageGiftLeft(p,st.id,passes.day)}번 더 받을 수 있어요</b>`:'<b>다 받았어요</b> · 다른 단계에 도전하면 또 받아요'}</div>`:''}<div class="sg-run-summary">${icon('mode_melee')} 기본 무기 1개 <span>+</span> ${skillIcon('F1')} 원소 스킬 4칸 <span>+</span> ${supportIcon('S4')} 지원품 4칸</div><button id="sg-start" class="sg-primary sg-start" data-do="start" ${passes?.remaining>0?'':'disabled'}>${passes?.remaining===0?'내일 아침 8시에 만나요':'출동하기'} ${icon('arrow')}</button><small class="sg-pass-hint">${duration===180?'첫 성공까지 3분':R.isBossStage(st.id)?'4분 뒤 대장 등장 · 최대 6분':'5분 도전'} · 성공할 때 이용권 1장</small></div></div>
  ...
  hardArea(p,stageId,value){
    ...
    if(!g.open)return `<p class="sg-hard-line lock">🔒 어려움 열기: ${esc(R.hardGateText(g))} <button class="sg-inline" data-do="hard-gate">자세히</button></p>`;
  ...
  hardGateDialog(stageId){
    ...
    const rows=g.items.map(it=>it.key==='stage'?`<tr class="${it.ok?'ok':'short'}"><td>${esc(it.label)}</td><td><b>${it.ok?'성공':'아직'}</b></td><td>보통 이상</td><td>${it.ok?'✓':'먼저 도전'}</td></tr>`:`<tr class="${it.ok?'ok':'short'}"><td>${esc(it.label)}</td><td><b>${it.now}</b>${esc(it.unit)}</td><td>${it.need}${esc(it.unit)}</td><td>${it.ok?'✓':`${it.need-it.now}${esc(it.unit)} 더`}</td></tr>`).join('');
  ...
  friends(p){
    ...
    const card=id=>{
      ...
      const spText=tier?[sp.text[0],tier>=2?`에픽: ${sp.text[1]}`:'',tier>=3?`전설: ${sp.text[2]}`:''].filter(Boolean).join(' · '):`유니크(${R.CARD_COPIES[2]}장)부터 · ${sp.text[0]}`;
      const btn=choose?`이 친구 고르기${pending.qty>1?` · 카드 ${pending.qty}장`:''}`:on?'함께 출동 중':owned?'함께 출동하기':'보급에서 만나요';
      return `<article class="sg-panel sg-pet-card ${on?'sg-equipped':''} ${owned?'':'sg-unowned'} ${choose?'sg-pet-choose':''}" style="--pet-color:${d.color}"><button class="sg-pet-pic" data-pet-detail="${id}" aria-label="${esc(d.name)} 자세히">${petImage(id)}</button><div class="sg-pet-body"><div class="sg-pet-name"><h2>${esc(d.name)}</h2>${owned?`<span class="sg-medal sg-medal-${g}">${R.GRADE_NAMES[g]}</span>`:''}<small class="sg-pet-role"><b>${d.no}</b>${esc(d.role)}</small></div>${owned?`<div class="sg-pet-meter"><span><i style="width:${pct}%"></i></span><small>${c}장${next?` · ${R.GRADE_NAMES[R.cardGrade(next)]}까지 ${next-c}장`:' · 최고 등급'}</small></div>`:''}<p class="sg-pet-buff">${esc(R.petBuffText(id,Math.max(0,g)))}</p><p class="sg-pet-sp ${tier?'on':''}"><b>${tier?'':'🔒 '}${esc(sp.name)}</b> ${esc(spText)}</p><div class="sg-pet-actions"><button class="${choose?'sg-primary':''}" ${owned||choose?'':'disabled'} data-action="${choose?'choose-pet':'pet'}" data-id="${id}">${btn}</button><button class="sg-inline" data-pet-detail="${id}">자세히</button></div></div></article>`;};
    ...
    <p class="sg-footnote">같은 친구 카드 ${R.GRADE_NAMES.map((n,i)=>`${n} ${R.CARD_COPIES[i]}장`).join(' → ')}. 버프는 전설(최대)의 ${R.GRADE_NAMES.map((n,i)=>`${n} ${Math.round(R.PET_GRADE_RATE[i]*100)}%`).join(' · ')}. <button class="sg-inline" data-do="pet-help">친구 규칙</button></p>`;
  }
  petDetail(id){
    const p=this.state().profile,d=R.PETS[id];if(!d||!p)return;const c=R.petCopies(p,id),g=R.petGrade(p,id),on=p.activePet===id;
    const grades=R.GRADE_NAMES.map((n,i)=>`<p class="sg-grade-row ${i===g?'on':''}"><span class="sg-medal sg-medal-${i}">${n}</span><span>${R.CARD_COPIES[i]}장 · ${esc(R.petBuffText(id,i))}${i===g?' <b>← 지금</b>':''}</span></p>`).join('');
