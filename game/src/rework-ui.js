...
export class GuardianUI {
  ...
  parts(p,passes){
    ...
    <div class="sg-section-heading"><h2>파츠 보관함</h2><small>${ownedKinds(p).length} / 10종 보유</small></div>${this.elementFilter(this.partFilter,'part-filter')}<div class="sg-grid sg-parts-grid">${entries(R.PARTS).filter(([,d])=>this.partFilter==='all'||d.element===this.partFilter).map(([id,d])=>{const item=p.parts[id],on=equipped.includes(id),cost=item?R.partUpgradeCost(item.level):null,sid=partSkill(d),full=equipped.length>=3;return `<article class="sg-panel sg-part-card ${on?'sg-equipped':''} ${item?'':'sg-unowned'}" style="--element:${R.ELEMENTS[d.element]?.color||'#4c91ae'}"><div class="sg-panel-title">${partIcon(id)}<div><span class="sg-eyebrow">${elementName(d.element)} · ${item?gradeName(item.copies):'미보유'}</span><h3>${esc(d.name)}</h3></div>${item?medal(item.copies):''}</div><p class="sg-part-skill">${skillIcon(sid)} ${esc(R.SKILLS[sid]?.name)} 파츠</p><p>${esc(desc(d))}</p>${item&&R.grade(item.copies)>=2?`<p class="sg-gold-feature">유니크 기능 · ${esc(d.gold)}</p>`:''}${item?`<div class="sg-part-stats"><b>Lv.${item.level} / 10</b><span>피해 +${partBonus(item)}%</span><small>${item.copies}개${toNext(item.copies)?` · ${toNext(item.copies).name}까지 ${toNext(item.copies).left}개`:' · 최고 등급'}</small></div><div class="sg-part-buttons"><button data-action="${on?'unequip-part':full?'swap-part':'equip-part'}" data-id="${id}">${on?'장착 해제':full?'바꿔 끼우기':'장착'}</button><button data-action="upgrade-part" data-id="${id}" ${cost!==null&&p.coins>=cost?'':'disabled'}>${cost===null?'최고 레벨':`레벨 올리기 · ${cost}`}</button></div>`:'<div class="sg-part-unowned">보급으로 받기 · 스킬은 지금도 사용 가능</div>'}<div class="sg-card-links"><button class="sg-inline" data-detail="${id}">기능·다음 효과</button>${item&&item.level>1?`<button class="sg-inline" data-reset="${id}">레벨 되돌리기</button>`:''}</div></article>`;}).join('')}</div><p class="sg-footnote">같은 파츠를 모으면 ${R.GRADE_NAMES.map((n,i)=>`${n} ${R.GRADE_COPIES[i]}개`).join(' → ')}. 등급마다 그 스킬 피해 +6%, 유니크부터 유니크 기능. 레벨 1단계마다 그 스킬 피해 +3%, 레벨 되돌리기를 하면 쓴 코인을 모두 돌려받아요.</p>`;
  }
  ...
  supplyHelp(){
    this.openDialog(`<h2>파츠 보급 규칙</h2><div class="sg-rules"><p><b>보급</b> 보급권 1장으로 10종 중 무작위 파츠를 받아요(전설이 된 파츠는 빼고, 남은 파츠는 모두 같은 확률). 고를 수는 없어요.</p><p><b>개수</b> ${R.SUPPLY_BUNDLES.map(b=>`${b.qty}개 ${Math.round(b.chance*100)}%`).join(' · ')} — 운이 좋으면 한 번에 여러 개!</p><p><b>등급</b> 같은 파츠를 모으면 올라가요. ${R.GRADE_NAMES.map((n,i)=>`${n} ${R.GRADE_COPIES[i]}개`).join(' → ')}. 등급마다 그 스킬 피해 +6%(전설 +24%), 유니크부터 파츠마다 유니크 기능이 생겨요.</p><p><b>전설</b> 아주 오래 모아야 해요. 매일 열심히 해도 두 달쯤 걸려요.</p><p><b>보급권 받는 곳</b> 성공 보상(쉬움·보통 1장, 어려움 2장, 같은 단계는 하루 2번 성공까지), 실패 격려(하루 1장까지), 코인 ${R.SUPPLY_EXCHANGE_COST}개 교환(하루 1번). 결제·광고는 없어요.</p></div><button class="sg-primary" data-close>확인</button>`);
  }
