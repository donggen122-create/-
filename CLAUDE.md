...
## 2026-09-24 밤. 친구 4종·등급·특수 능력 + 일일 미션(배포 전) — `docs/37_친구_4종_일일_미션.md`
- 친구 카드 `petCopies`(1/3/7/25/80장 등급), 우정 없음, 친구 보급 `draw-pet`. 미션 `MISSIONS` 5개 × 2장(서버 날짜로 정산·훈련에서 셈, 바로 지급). 어려움 실패 권장치·세트 창 줄바꿈(f6a704f)과 함께 아직 실서버 배포 전 — "배포해"를 기다린다.
