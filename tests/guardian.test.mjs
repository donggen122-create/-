...
test('first part, drawing, upgrades and refunds are idempotent across duplicate requests',async()=>{
 ...
 for(let i=0;i<4;i++)await api(env,'/guardian/action',{requestId:uid(),kind:'draw-part',element:i%2?'fire':'water'});
 const draw={requestId:uid(),kind:'draw-part',id:'PART_L2'};first=await api(env,'/guardian/action',draw);dup=await api(env,'/guardian/action',draw);assert.equal(dup.profile.parts.PART_L2.copies,1);assert.equal(dup.profile.gifts,5);assert.equal(dup.profile.giftCounts.part,5);
