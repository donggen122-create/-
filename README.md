...
서호초등학교 학생용 브라우저 게임. 실서버: https://seoho-pangpang.seoho-pangpang-server.workers.dev · 관리 페이지: `/admin/`

**다른 컴퓨터에서 작업하기**: Git·Node.js를 설치하고 이 브랜치를 내려받은 뒤(`git clone -b seoho-game https://github.com/donggen122-create/-.git seoho-game`) Claude에게 "setup 해 줘"라고 하면 됩니다(`game/tools/setup.ps1`). 작업 규칙은 [`CLAUDE.md`](CLAUDE.md), 인수인계는 [`HANDOFF.md`](HANDOFF.md), 작업 기록은 [`game/STATUS.md`](game/STATUS.md). 실서버 배포는 `game/tools/deploy.ps1`(Cloudflare, PC마다 `npx wrangler login` 한 번).

아래는 처음 만든 기획서(LUMEN) 안내입니다.
